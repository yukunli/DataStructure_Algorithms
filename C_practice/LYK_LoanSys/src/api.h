...
typedef enum
{
    ...
    OP_E_IDX           = 3,            //贷款编号输入错误      E002:贷款编号输入错误
    OP_E_SALARY        = 4,            //贷款人月收入输入错误  E003:贷款人月收入输入错误
    OP_E_PRINCIPAL     = 5,            //贷款本金输入错误		E004:贷款本金输入错误 
    OP_E_YEAR          = 6,            //贷款年限输入错误		E005:贷款年限输入错误 
    OP_E_APPLY_TIME    = 7,            //申请时间输入错误		E006:申请时间输入错误
    OP_E_APPLYTIME_SUB = 8,            //申请时间错误递减		E007:申请时间错误递减
    OP_E_MODIFY        = 9,            //贷款处理正常,不允许修改初始信息	E008:贷款处理正常,不允许修改初始信息
    OP_E_SUCCESS       = 10,           //贷款申请成功			E009:贷款申请成功
    OP_E_NO_VALIDAPPLY = 11,           //无有效的贷款申请		E010:无有效的贷款申请
	OP_E_MONTH         = 12,           //还款月份无效			E011:还款月份无效
    OP_E_FUND_MONEY    = 13,           //住房公积金还款金额无效	E012:住房公积金还款金额无效
    OP_E_QRY_TIME      = 14,           //查询时间输入错误		E013:查询时间输入错误
    OP_E_QRYTIME_SUB   = 15,           //查询时间错误递减		//E014:查询时间错误递减
...
typedef struct
{
	...
	int principal;      //贷款本金(万元)
	int monthAll;       //还款期数
	int applytime;		//贷款申请时间(t)
	int againtime;		//待重新申请的时间
	int waitgrandtime;	//待发放的时间(指明当前状态所在的t)
	int grandtime;		//发放完成的时间
}LoanHistoryInfoItem;
...
typedef struct
{
	int iSystemTime;			// 系统时间
	int liDoneLoanMoney ;		//已发放贷款金额
	int iDoneLoanNum ;			//已发放贷款份数
}LoanSystemInfo;
/*****************************************************************************
